<template>
  <div>
    <v-container fill-height fluid style="padding-bottom: 0px;">
      <v-row style="padding-top: 12px; padding-bottom: 12px">
        <v-col style="padding-bottom: 0px;">
          <DPlayer :article="parseInt(videoId)" :video="videoData.video[0]" :picurl="videoData.imgUrl" />
        </v-col>
      </v-row>
    </v-container>
    <v-container fill-height style="padding-top: 0px;">
      ...
      <v-row style="padding-top: 12px; padding-bottom: 12px">
        <v-col :cols="colsWidth">
          <Comment :article="videoId" :author-id="videoData.userId" :typenum="1" :count="videoData.commentCount" />
        </v-col>
        <v-col>
          相关视频：
        </v-col>
...
export default {
  ...
  methods: {
    ...
    videoInfo() {
      fetch(`/api/article/video/${this.videoId}`, {
        headers: {
